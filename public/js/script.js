...
const startTest = async () => {
  ...
  while (count < 5) {
    ...
    } while (selectedIndices.includes(number));
    selectedIndices.push(number);
    count++;
  }
...
const nextQuestion = () => {
  if (questionIndex < selectedIndices.length - 1) {
    questionIndex++;
    let questionObj = questions[selectedIndices[questionIndex]];
    //console.log(questionObj);
...
const renderAllQuestionsAndAnswers = () => {
  ...
  for (let i = 0; i < selectedIndices.length; i++) {
    let questionObj = questions[selectedIndices[i]];
    ...
    let personality = questionObj.answers[answer].personality;
